...
contract LiquidationOperator2 is IUniswapV2Callee {
    ...
    constructor() {
        ...
        // debt_USDT = 2916378221684; Parsing input instead from liquidation2.js
    ...
    function operate(uint256 _USDT_contract_input) external {
        ...
        debt_USDT = _USDT_contract_input; // parsing in put from liquidation2.js
    ...
    function uniswapV2Call(
        ...
        (uint256 reserve_WBTC, uint256 reserve_USDT, ) = uniswapV2Pair_WBTC_USDT.getReserves(); //pool1
        ...
        // 2.3 repay with WBTC directly without swapping
        uint repay_WBTC = getAmountIn(debtToCover, reserve_WBTC, reserve_USDT);
        WBTC.transfer(address(uniswapV2Pair_WBTC_USDT), repay_WBTC);
        
        // 2.4 convert to WETH to check profit!!!
        uint afterRepay_WBTC = WBTC.balanceOf(address(this));
        WBTC.transfer(address(uniswapV2Pair_WBTC_WETH), afterRepay_WBTC);
        console.log("check profit");
        uint amountOut_WETH = getAmountOut(afterRepay_WBTC, reserve_WBTC_Pool2, reserve_WETH_Pool2);
        uniswapV2Pair_WBTC_WETH.swap(0, amountOut_WETH, address(this), "");
